...
export default function ClientesList() {
  ...
  useEffect(() => {
    // Cuando cambian estos filtros directo que lance la query si no es search
    loadClientes();
  }, [page, estado]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button className="bg-indigo-600 text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 transition font-medium flex items-center gap-2">
                  ...
                  <tr key={cliente.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        📋 {cliente.proyectos_totales || 0} proyectos
                      </div>
                      <div className="text-sm text-green-600 font-medium mt-1">
                        💰 ${(cliente.total_invertido || 0).toLocaleString()} USD
                      </div>
                    </td>
